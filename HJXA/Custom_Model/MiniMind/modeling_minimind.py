...
class MiniMindConfig(PretrainedConfig):
    ...
    def __init__(
            ...
            max_position_embeddings: int = 32768, # Qwen2 也是
...
# 把少量 KV 头“广播复制”成更多头，用于匹配 Query 头数量。
def repeat_kv(x: torch.Tensor, n_rep: int) -> torch.Tensor:
    """torch.repeat_interleave(x, dim=2, repeats=n_rep)"""
    # expand + reshape 更高效 repeat_interleave 会真的复制数据 expand 只是共享内存
...
class Attention(nn.Module):
    def __init__(self, args: MiniMindConfig):
        ...
        # ======源代码
        # self.n_local_heads = args.num_attention_heads # 当前本地的头数
        # self.n_local_kv_heads = self.num_key_value_heads # 当前本地的kv数
        ...
        self.q_proj = nn.Linear(args.hidden_size, args.num_attention_heads * self.head_dim, bias=False) # 去除QKV的偏置
    ...
    def forward(self,
        ...
        if past_key_value is not None:
            # 要推理更快，可以参考Qwen2的源代码有个cache_position
        ...
        if self.flash and (seq_len > 1) and (past_key_value is None) and (attention_mask is None or torch.all(attention_mask == 1)):
            # attention_mask 掩码逻辑是否**“足够简单”**（不涉及复杂的 Padding 遮掩），scaled_dot_product_attention自带因果掩码（is_causal=True））
            output = F.scaled_dot_product_attention(xq, xk, xv, dropout_p=self.dropout if self.training else 0.0, is_causal=True)
        else:
            scores = (xq @ xk.transpose(-2, -1)) / math.sqrt(self.head_dim)
            # 因果编码
            # 使用 -seq_len: 是为了只给当前输入的新 Token 之间加上因果遮蔽，而保留它们对历史 KV Cache 的完整可见性。（兼容推理）
            scores[:, :, :, -seq_len:] += torch.triu(torch.full((seq_len, seq_len), float("-inf"), device=scores.device), diagonal=1)

            # Padding Mask
            if attention_mask is not None: # mask中1 是我们要的词，0 是 Padding
        ...
        output = output.transpose(1, 2).reshape(bsz, seq_len, -1) # 头合并
...
class MiniMindBlock(nn.Module):
    def __init__(self, layer_id: int, config: MiniMindConfig):
        ...
        self.mlp = FeedForward(config) if not config.use_moe else MOEFeedForward(config) # 是否用MoE
